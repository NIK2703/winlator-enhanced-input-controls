...
static inline void toggle_slide_over(TouchElement* btn, TouchActionResult* restrict result, uint64_t time_ms) {
    if (btn->selected) {
        ...
        // Check BEFORE release_toggled_alternate_bindings — slide-over must not clear
        // gesture_toggled/lp_toggled; only the user's deliberate re-gesture can do that.
            ...
            if (tb->count > 0) {
                ...
                if (curr_idx != prev_idx) {
                    ...
                    if (prev_idx >= 0 && prev_idx < g_state.element_count) {
                        ...
                        if (hover_f) finger_remove_engaged(hover_f, (int16_t)prev_idx);
                    }
                    // Activate current
                    if (btn && btn->type == ELEM_BUTTON) {
                        if (btn->cached_has_toggle && !btn->gesture_timer_armed) {
